...
class symbiosity: public application<T,R> {
  public:
    size_t count;
    double N; // Threshold

    symbiosity(Parser input_data):
    ...
    common::stopClock("symbiosity", start_time);
...
static application<T,R>* init_app(Parser input_data){
  return new symbiosity<T,R>(input_data);
}
