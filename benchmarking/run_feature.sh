...
for system in "overhead"; do
  ...
  for run in `seq $numruns`; do
   for dataset in $datasets; do
     ${STABLE_BIN_DIR}/undirected_triangle_counting_noalgo --graph=/dfs/scratch0/caberger/datasets/${dataset}/bin/u_degree.bin --input_type=binary --t=1 --layout=uint | tee $odir/${dataset}_${run}_pruned_uint.log
   done
  done
done
