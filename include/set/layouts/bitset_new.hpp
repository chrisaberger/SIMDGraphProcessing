...
inline size_t bitset_new::word_index(const uint32_t bit_index){
  return bit_index >> ADDRESS_BITS_PER_WORD;
}
//check if a bit is set
inline bool bitset_new::is_set(const uint32_t index, const uint64_t * const in_array, const uint64_t start_index){
  return (in_array)[word_index(index)-start_index] & ((uint64_t) 1 << (index%BITS_PER_WORD));
}
//check if a bit is set
inline void bitset_new::set(const uint32_t index, uint64_t * const in_array, const uint64_t start_index){
...
inline size_t bitset_new::build(uint8_t *R, const uint32_t *A, const size_t s_a){
  if(s_a > 0){
    size_t i = 0;
...
inline size_t bitset_new::build_flattened(uint8_t *r_in, const uint32_t *data, const size_t length){
  if(length > 0){
    common::num_bs++;
...
template<typename F>
inline void bitset_new::foreach_until(
    F f,
    const uint8_t *A,
    const size_t cardinality,
    const size_t number_of_bytes,
...
//Iterates over set applying a lambda.
template<typename F>
inline void bitset_new::foreach(
    F f,
...
template<typename F>
inline size_t bitset_new::par_foreach(
      F f,
      const size_t num_threads,
      const uint8_t* A,
      const size_t cardinality,
