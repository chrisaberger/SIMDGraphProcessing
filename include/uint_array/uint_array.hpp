...
namespace uint_array{
  ...
  static size_t size_of_array(uint8_t* data, size_t cardinality, common::type t) {
    size_t size = 0;
    #if HYBRID_LAYOUT == 1
    t = (common::type) data[0];
    data++;
    size += sizeof(common::type);
    #endif

    if(t == common::ARRAY32) {
       size += cardinality * sizeof(uint32_t);
    }
    else {
       size += ((size_t*)data)[0] + sizeof(size_t);
    }
    return size;
  }
}
